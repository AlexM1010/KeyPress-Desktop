<script lang="ts">
  ...
  import Flow from '../../workspace/Flow.svelte'; // Import Flow component
  import { SvelteFlowProvider } from "@xyflow/svelte";

  export let data: { project?: Project };

  import { nodesData, edgesData } from "$lib/stores/flow";
    ...
    <div class="flex flex-col items-center overflow-x-hidden">
      <Banner img={getAssetURL(data.project.logo)}>
        <div class="col-center p-y-20">
          ...
          <p class="font-600 text-center text-2xl my-2">{data.project.name}</p>
          {#if $nodesData.length > 0}
            <div class="small-flow">
              <SvelteFlowProvider>
                <Flow previewMode={true} nodes={nodesData} edges={edgesData} />
              </SvelteFlowProvider>
            </div>
          {/if}
          <div class="w-75%">
...
<style lang="scss">
  ...
  .small-flow :global(.flow-container) {
    height: 100%;
  }
</style>
